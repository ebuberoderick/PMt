                ...
                <div class="download">
                    <div class="row">
                        <div class="col-6">
                            ...
                            <div class="mt-2">Vehicle type</div>
                            <div class="mt-2">Number of booked seats</div>
                            <div class="mt-2">Time booked</div>
                            <div class="mt-2">Amount paid</div>
                            <div class="mt-2">Departure time</div>
                        </div>
                            <div class="col-6">
                                ...
                                <div class="mt-2" id="bustype">: </div>
                                <div class="mt-4 pb-2" id="seats">: </div>
                                <div class="mt-2" id="timeBook">: </div>
                                <div class="mt-2" id="price">: </div>
                                <div class="mt-2" id="departure">: </div>	 	
                            </div>
                    </div>
    ...
    <div style="display:flex;justify-content:center;align-items:center;background:linear-gradient(to top,#2b2d60,#2b2d60da,#2b2d60b6,#dd7723);padding:4rem" class="center">
        <div class="form_cardd card p-3 z-depth-3" data-aos-duration="2000" data-aos="zoom-out" style="margin-top:80px;min-width:350px;max-width:400px">
            <form method="post" name="ticket" id="paymentForm" accept-charset="UTF-8">
                ...
                <div class="input-group mb-3 input-field" style="width:100% !important">
                    <select name="" class="browser-default Vehicle">
                        <option selected Disabled>Vehicle Type</option>
                    </select>
                </div>
                <div style="display:">
                    <div class="price mb-3 left-text">
                        Number Of Available Seats : <span class="seats flow-text"></span>
                    </div>
                    <div class="price mb-3">
                        Pick SEAT(s):
                        <div class="input-group mb-3 form-controler" style="width:100% !important">
                            <select name="" class="browser-default seatval">
                            </select>
                        </div>
                    </div>
                    <div style="display:">
                        <div class="price mb-3">
                            PRICE:
                            <div class="flow-text totalPrice">
                                NGN 0.00
                            </div>
                        </div>
                        <input type="hidden" id="totalPrice">
                    </div>
                    <button type="submit" onclick="payWithPaystack()" class="btn btn-peace mb-3 book btnre disabled">Book Now</button>
                </div>
            </form>
        </div>
    </div>
        ...
        function payWithPaystack(e) {
            ...
            let handler = PaystackPop.setup({
                ...
                callback: function(response) {
                    ...
                    $showv = $('.Vehicle').val();
                    $showg = $('.seatval').val();
                    $showe = $('#totalPrice').val();
                    $encyData= $('.userid').val();
                    $.ajax({
                        type: 'post',
                        url: 'paySuccessful.php',
                        data: { 'from':$shown,'to':$showt,'Vehicle':$showv,'seatval':$showg,'totalPrice':$showe,'ency':$encyData },
                        cache: false,
                        dataType:'json',
                        success: function(returnData) {
                            if(returnData.status == 'success'){
                                ...
                                $('#bustype').text(`: ${returnData.bustype}`);
                                $('#seats').text(`: ${returnData.seats}`);
                                $('#timeBook').text(`: ${returnData.timeBook}`);
                                $('#price').text(`: &#8358; ${returnData.price}`);
                                $('#departure').text(`: ${returnData.departure}`);
